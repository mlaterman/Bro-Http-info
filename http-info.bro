#TODO - rewrite to lessen # of record types needed
##Updated to work with the current (2.3) version of bro
##New record datatype for logging created (logging not fully implemented)
...
#NEW record for logger
type HTTP_INFO: record {
    conn_ID             : int    &default=+0 &log;
    client_id           : int    &default=+0 &log;
    res_id              : addr   &default=0.0.0.0 &log;
    pipelined           : bool   &default=F &log;
    req_ts              : time   &default=double_to_time(0.0) &log;
    rep_s_ts            : time   &default=double_to_time(0.0) &log;
    rep_e_ts            : time   &default=double_to_time(0.0) &log;
    bytes               : int    &default=+0 &log;
    user_agent          : string &default="-" &log;
    method              : string &default="-" &log;
    version             : string &default="-" &log;
    host                : string &default="-" &log;
    content_length_up   : string &default="-" &log;
    content_length_down : string &default="-" &log;
    content_type        : string &default="-" &log;
    code                : count  &default=0 &log;
    server_retrans      : count  &default=0 &log;
    no_cache            : bool   &default=F &log;
    no_store            : bool   &default=F &log;
    public              : bool   &default=F &log;
    location            : string &default="-" &log;
    referer             : string &default="-" &log;
    sanity              : string &default="ND" &log;
    uri                 : string &default="-" &log;
};

global log_HTTPINFO: event(rec: HTTP_INFO);

global UofC: subnet 136.159.0.0/16;
...
# First argument is if there was an error making the string (based on if statement)
function http_tostring(id:conn_id,transaction: http_info):string {
    if(id !in info)
        return "ERR CONNECTION NOT IN INFO";
    ...
    if(transaction$reply_end_seq > 0 && transaction$reply_start_seq > 0) {
        ...
        transaction$reply_end_seq - transaction$reply_start_seq,
...
function writeToLog(id:conn_id,transaction: http_info) {
    local rec: HTTP_INFO;
    if(id in info) {
	    local conn = info[id];
	    rec = [
                    $conn_ID = conn$connID,
                    $client_id = ClientID[id$orig_h],
			$res_id = id$resp_h,
			$pipelined = conn$pipelined,
			$req_ts = transaction$request_t,
			$rep_s_ts = transaction$reply_start_t,
			$rep_e_ts = transaction$reply_end_t,
			$user_agent = transaction$user_agent,
			$method = transaction$method,
			$version = transaction$version,
			$host = transaction$host,
			$content_length_up = transaction$content_length_up,
			$content_length_down = transaction$content_length_down,
			$content_type = transaction$content_type,
			$code = transaction$code,
			$server_retrans = transaction$server_retrans,
			$no_cache = transaction$no_cache,
			$no_store = transaction$no_store,
		...
		if(transaction$reply_end_seq > 0 && transaction$reply_start_seq > 0) {
		    rec$bytes = transaction$reply_end_seq - transaction$reply_start_seq;
		}
	}
	Log::write(HTTPINFO, rec);
}
...
event new_packet(c: connection, p: pkt_hdr) {
    ...
    if( p$tcp$flags == TH_RST || p$tcp$flags == TH_RST + TH_ACK ) {
        if(id !in info) {
        ...
        else if(info[id]$connstate == not_established || info[id]$connstate == waiting_ACK) {
            ...
            if(thisidx in info[id]$transactions) {
                ...
                print fmt("I|%s", http_tostring(id,info[id]$transactions[thisidx]));
...
event http_message_done(c: connection, is_orig: bool, stat: http_message_stat) {
    if (is_orig) # It's for the client rather than the server - ignore.
        return;
